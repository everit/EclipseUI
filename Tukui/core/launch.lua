...
local function install()
	...
	if (TukuiCF.chat.enable == true) and (not IsAddOnLoaded("Prat") or not IsAddOnLoaded("Chatter")) then					
		...
		for i = 1, NUM_CHAT_WINDOWS do
			...
			frame:SetSize(TukuiDB.Scale(TukuiCF["panels"].tinfowidth - 9), TukuiCF["chat"].chatheight - 9)
			
			-- this is the default width and height of tukui chats.
			SetChatWindowSavedDimensions(chatFrameId, TukuiDB.Scale(TukuiCF["panels"].tinfowidth - 9), TukuiCF["chat"].chatheight - 9)
			
			-- move general bottom left or Loot (if found) on right.
			if i == 1 then
				frame:ClearAllPoints()
				frame:SetPoint("BOTTOMLEFT", TukuiChatLeft, "BOTTOMLEFT", TukuiDB.Scale(4), TukuiDB.Scale(5))
