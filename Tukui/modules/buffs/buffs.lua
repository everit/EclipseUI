...
local function UpdateWeapons(button, slot, active, expiration)
	if not button.texture then
		...
		button.time:SetFont(C.media.pixel_font, 12, "MONOCHROMEOUTLINE")
...
local function UpdateAuras(header, button, weapon)
	if(not button.texture) then
		...
		button.count:SetPoint("TOPLEFT", 3, 1)
		button.count:SetFont(C["media"].pixel_font, 12, "MONOCHROMEOUTLINE")

		button.time = button:CreateFontString(nil, "ARTWORK")
		button.time:SetPoint("BOTTOM", 1, -14)
		button.time:SetFont(C["media"].pixel_font, 12, "MONOCHROMEOUTLINE")
...
ScanAuras(CreateAuraHeader("HELPFUL", "TOPRIGHT", -8, -8))
ScanAuras(CreateAuraHeader("HARMFUL", "TOPRIGHT", -8, -130))
