...
if UnitName("player") == "Foofqt" then return end

local CreateSpellEntry = function( id, castByAnyone, color, unitType, castSpellId, cooldownSpellId )
	return { id = id, castByAnyone = castByAnyone, color = color, unitType = unitType or 0, castSpellId = castSpellId, cooldownSpellId = cooldownSpellId }
end
...
local MASTER_FONT, STACKS_FONT = { C.media.caith, 11, nil }, { C.media.caith, 11, nil }
...
local CLASS_FILTERS = {
	...
	WARLOCK = { 
		target = {
			CreateSpellEntry( 48181, false, CreateColor( 102, 153, 102, 1 ), nil, nil, 48181 ), -- Haunt
			-- CreateSpellEntry( 32389 ), -- Shadow Embrace 
			CreateSpellEntry( 172 ), -- Corruption
			CreateSpellEntry( 30108 ), -- Unstable Affliction
			CreateSpellEntry( 603, false, CreateColor( 0, 51, 0, 1 ) ), -- Bane of Doom
			CreateSpellEntry( 980 ), -- Bane of Agony
			CreateSpellEntry( 80240 ), -- Bane of Havoc
			CreateSpellEntry( 1490, true, CreateColor( 72, 0, 113, 1 ) ), -- Curse of the Elements 
			-- CreateSpellEntry( 18018 ), -- Conflagration
			CreateSpellEntry( 348, false, CreateColor( 200, 154, 52, 1 ) ), -- Immolate
			CreateSpellEntry( 27243 ), -- Seed of Corruption
			-- CreateSpellEntry( 17800 ), -- Improved Shadow Bolt
		},
		player = { 
			-- CreateSpellEntry( 28176 ), -- Fel Armor
			CreateSpellEntry( 47241 ), -- Metamorphosis
		},
		procs = {
			...
			CreateSpellEntry( 89937 ), -- Fel Spark
		},
	},
	WARRIOR = { 
		target = {
...
do
	...
	local AddFilter = function( self, filter, defaultColor, debuffColor )
		...
		for _, v in pairs( filter ) do
			...
			clone.cooldownSpellId = v.cooldownSpellId
	...
	do
		...
		local OnUpdate = function( self, elapsed )	
			...
			if ( time > self.expirationTime ) then
				...
				if ( castSeparator and self.cooldownSpellId ) then
					local start, duration, enabled = GetSpellCooldown( self.cooldownSpellId )
					
					dura = start + duration - GetTime()
					
					if ( dura and remaining > dura and duration > 1.5 ) then
						castSeparator:SetPoint( "CENTER", self.bar, "LEFT", barWidth * ( dura ) / self.duration, 0 )
					else
						castSeparator:Hide()
					end
				end
		...
		local SetCooldownSpellId = function( self, id )
			self.cooldownSpellId = id
		...
		local SetAuraInfo = function( self, auraInfo )
			...
			self:SetCooldownSpellId( auraInfo.cooldownSpellId )
		end
		...
		CreateAuraBar = function( parent )
			...
			result.SetCooldownSpellId = SetCooldownSpellId
...
local cOffset = TukuiDB.Scale(11)
local uOffset = TukuiDB.Scale(0)
local iOffset = TukuiDB.Scale(1)

local targetDataSource = CreateUnitAuraDataSource( "target" )
local playerDataSource = CreateUnitAuraDataSource( "player" )
local petDataSource = CreateUnitAuraDataSource( "pet" )
...
petDataSource:SetSortDirection( SORT_DIRECTION )
...
if ( classFilter ) then
	...
	petDataSource:AddFilter( classFilter.pet, PLAYER_BAR_COLOR, PLAYER_DEBUFF_COLOR )
...
-- if TukuiCF["unitframes"].playerauras then
	-- if TukuiCF["ct"].icons == true then
		-- playerFrame:SetPoint( "BOTTOMLEFT", TukuiPlayer, "TOPLEFT", iOffset - 1, yOffset + cOffset )
	-- else
		-- playerFrame:SetPoint( "BOTTOMLEFT", TukuiPlayer, "TOPLEFT", uOffset, yOffset + cOffset )
	-- end
	-- playerFrame:SetPoint( "BOTTOMRIGHT", TukuiPlayer, "TOPRIGHT", -uOffset, yOffset + cOffset )
-- else
	-- if ( playerClass == "DEATHKNIGHT" and TukuiCF["unitframes"].runebar == true ) or ( playerClass == "SHAMAN" and TukuiCF["unitframes"].totemtimer == true ) or ( playerClass == "DRUID" or playerClass == "WARLOCK" or playerClass == "PALADIN" ) then
		playerFrame:SetPoint( "BOTTOMLEFT", TukuiPlayer, "TOPLEFT", BAR_HEIGHT + 7, 17 )
		playerFrame:SetPoint( "BOTTOMRIGHT", TukuiPlayer, "TOPRIGHT", 0, 17 )
		-- playerFrame:SetScale(0.75)
	-- else
		-- playerFrame:SetPoint( "BOTTOMLEFT", TukuiPlayer, "TOPLEFT", iOffset + 21, yOffset - uOffset )
		-- playerFrame:SetPoint( "BOTTOMRIGHT", TukuiPlayer, "TOPRIGHT", -uOffset, yOffset - uOffset )
	-- end
-- end
playerFrame:Show()

local petFrame = CreateAuraBarFrame( petDataSource, TukuiPlayer )
petFrame:SetHiddenHeight( -yOffset )
petFrame:SetPoint( "BOTTOMLEFT", playerFrame, "TOPLEFT", 0, yOffset )
petFrame:SetPoint( "BOTTOMRIGHT", playerFrame, "TOPRIGHT", 0, yOffset )
petFrame:Show()
...
trinketFrame:SetPoint( "BOTTOMLEFT", petFrame, "TOPLEFT", 0, yOffset )
trinketFrame:SetPoint( "BOTTOMRIGHT", petFrame, "TOPRIGHT", 0, yOffset )
trinketFrame:Show()
-- trinketFrame:SetScale(0.75)
